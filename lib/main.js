...
function dayOfYear( month, day, year ) {
	...
	if ( arguments.length === 0 ) {
		...
		if ( isDateObject( month ) ) {
			...
			throw new TypeError( format( '1Q9AW', month ) );
		}
	} else {
		if ( !isString( month ) && !isInteger( month ) ) {
			throw new TypeError( format( '1Q9AX', month ) );
		}
		if ( !isInteger( day ) ) {
			throw new TypeError( format( '1Q97f', day ) );
		}
		if ( !isInteger( year ) ) {
			throw new TypeError( format( '1Q92z', year ) );
		}
		mon = month;
		d = day;
		yr = year;
	...
	if ( d < 1 || d > days ) {
		throw new RangeError( format( '1Q9Dz', days, d ) );
	}
