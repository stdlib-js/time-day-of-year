...
function dayOfYear( month, day, year ) {
	...
	if ( arguments.length === 0 ) {
		...
		if ( isDateObject( month ) ) {
			...
			throw new TypeError( format( '1Q9AW,La', month ) );
		}
	} else {
		if ( !isString( month ) && !isInteger( month ) ) {
			throw new TypeError( format( '1Q9AX,LV', month ) );
		}
		if ( !isInteger( day ) ) {
			throw new TypeError( format( '1Q97f,LW', day ) );
		}
		if ( !isInteger( year ) ) {
			throw new TypeError( format( '1Q92z,GL', year ) );
		}
		mon = month;
		d = day;
		yr = year;
	...
	if ( d < 1 || d > days ) {
		throw new RangeError( format( '1Q9Lb', days, d ) );
	}
